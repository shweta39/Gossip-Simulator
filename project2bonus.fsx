...
let killNode = int fsi.CommandLineArgs.[4]
        ...
        actor {
            ...
            |GossipNode (index, message) ->
                if nodeCount < 10 then 
                    if nodeCount = 9 then 
                        printfn "\nFinished Node: %d" id // this node is finished 
                        system.ActorSelection("user/GossipParent")
        ...
        let rec parentLoop parentCount () =
            actor {
                ...
                | StartGossipParent (inputNodes, inputTopology, inputAlgo) ->
                    printfn "\nTOPOLOGY = %s \nNUMBER OF NODES = %i \nALGORITHM = %s\nKILL NODES = %i" inputTopology inputNodes inputAlgo killNode
                ...
                | EndPushSumParent ("Done", index, ratio) -> //End Push sum algorithm 
                    ...
                    counter <- counter + 1
                    if parentCount = inputNodes then //if counter reached the end of node
                        printfn "\n\nAll nodes are converged\n\n"
                        endFlag <- true
...
let main(args) = 
    ...
    while not endFlag  // end the program if end flag turns true otherwise ignore
        && (((DateTime.Now - start)).TotalMinutes < 1.00) do
        ignore ()
...
timer.Stop()
printfn "\nTime taken = %f ms"  timer.Elapsed.TotalMilliseconds
printfn "\nTotal nodes converged: %d \n" counter
